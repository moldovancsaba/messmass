...
# Generated by `next build`/`next dev` (gitignored); it embeds a nextjs.org link, so
# scanning it after a local build reported a phantom outbound host (savetheworld).
...
def source_files(root: Path, exts: tuple[str, ...]) -> list[Path]:
    ...
    for path in root.rglob("*"):
        if path.suffix not in exts or not path.is_file() or path.name in SKIP_FILES:
            continue
