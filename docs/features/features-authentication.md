...
**Where the password prompt appears** (verified against code @ v12.3.26, 2026-09-07):

| Page type | Page | Prompt rendered by | Data route enforcement |
|---|---|---|---|
| `event-report` | `/report/[slug]` | server layout `app/report/[slug]/layout.tsx` → `ServerPageGate` (added v12.3.26; before that the page had **no prompt at all**, so a protected share link crashed with a JSON-parse message) | `requirePageAccess('event-report', slug)` in `GET /api/projects/stats/[slug]` |
