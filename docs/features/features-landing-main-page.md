...
- **Landing UI refactor (Phases 1–5):** Single content width via theme tokens (`--mm-content-max-width`, `--mm-content-padding-x`); hero, sections, report block, footer use one container pattern. Static snapshot includes `styleId`; LandingPageStatic calls `useReportStyle(snapshot.styleId)` so the report style (e.g. admin/styles/:id) drives the main page. Cards and section headings use `--chartBackground`, `--chartBorder`, `--chartTitleColor`. Report page `.container` uses the same tokens (Phase 4). Shared UI primitives are documented in the refactor plan (Phase 5). See `docs/landing-main-page-ui-refactor-plan.md` and MVP Factory Board #248.
- **v11.58.0:** Style editor drives all landing visuals. Report styles gain 9 landing color fields (hero gradient, hero text/muted/border, page background) and 13 dimension fields (section/hero padding, card min widths, FAQ/paragraph max width, landing icon size, card border radius and shadow, landing block title/value/icon font sizes). When a style is applied (static snapshot or live), landing and report use these variables with theme fallbacks. Admin → Styles → [style] shows a "Dimensions & surfaces" section; values are stored with the style and injected as CSS variables. See `lib/reportStyleTypes.ts` (DIMENSION_FIELDS, injectStyleAsCSS), `docs/release-notes-11.58.0.md`.
- **v11.59.0:** Style editor preview refreshes on every change (inject on change + after fetch). Bar and pie colors use CSS variables so report and preview stay in sync. Value Chain block and Landing page section added to style editor preview. See `docs/release-notes-11.59.0.md`.
