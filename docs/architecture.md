...
- **Password Protection**: server component checks `isPageProtected`/`hasPageAccess` (`lib/pageAccess.ts`) and renders `ServerPageGate` before any data is read. `/report/[slug]` applies the identical gate in `app/report/[slug]/layout.tsx` (v12.3.28); its data route `GET /api/projects/stats/[slug]` enforces via `requirePageAccess`
...
**Command**: none — the `seed:variables` script was removed in v12.3.28 (scripts prune, messmass#352); variables are managed in `/admin/kyc`.
