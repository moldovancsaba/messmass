...
- `GET /api/v3/activities/{id}`: **Not implemented.** No route exists at this path (only `/{id}/participants`); requests get Next's HTML 404. The `useReportData` fallback that called it was removed in v12.3.25 after it masked every protected-report error as a JSON parse failure.
...
Event reports (`/report/[slug]`) load V2 projects only; the V3 Activity bridge in `useReportData` was removed in v12.3.25 because its target route never existed. Partner reports (`usePartnerReportData` → `/api/v3/entities/{id}`) and organization reports still bridge V3 data through `lib/v3/compatAdapter.ts`.
