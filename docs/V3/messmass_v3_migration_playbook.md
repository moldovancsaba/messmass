...
3.  **Fallback Engine**: `usePartnerReportData` tries the V2 Partner lookup first, then falls back to `/api/v3/entities/{id}`. The equivalent V3 Activity fallback in `useReportData` was removed in v12.3.24 — `GET /api/v3/activities/{id}` was never built, so the fallback only ever produced an HTML 404 that masked the real error.
