...
- `v12.1.20` — Styled-jsx migration (#85 part 2): migrated `UnifiedHashtagInput` to a CSS module, classified the visualization page's runtime-grid styled-jsx as an intentional exception, and held `ChartAlgorithmManager`/`ChartConfiguration` for visual QA (they intermix global `.btn` design-system classes with local overrides).
