...
## [v12.3.30] - 2026-09-07T16:00:00.000Z — A Dead API Fallback Masked Every Protected-Report Error
...
### Context
A user reported a single visible UI bug (example: unreadable text in one specific input). The first fix was applied only to that one place, but the same pattern existed elsewhere.

### What Went Wrong
That was a **single-point patch**: treating a symptom as an isolated incident instead of recognizing it as a systemic pattern.

This is unprofessional because:
- the same issue likely exists elsewhere,
- users must report each instance,
- it produces duct-tape fixes instead of addressing the underlying class of problems.

### Protocol (How We Avoid This Now)
For any bug or UI/design issue:
1. Infer the root cause and the pattern/class.
2. Search the codebase for all instances of that pattern (same component type, same context, same anti-pattern).
3. Fix all instances of the class, **or** explicitly document scope + what remains (with follow-ups).
...
### Context
Export CSV/PDF can fail silently (missing DOM targets for PDF capture, missing/invalid data, or caught errors that never reach the user).

### Rule
Exports must be treated like a user-facing workflow:
1. Validate prerequisites before attempting export (data readiness, required DOM IDs/markers for PDF capture).
2. If export fails, show a clear error message in the UI (not just console logs).
3. Prefer a single shared export path/hook so CSV/PDF follow the same validation + error handling contract.
...
remote:   GitHub Personal Access Token detected in docs/operations/WARP.md:7
```

**What went wrong**:
- Added GitHub token to docs/operations/WARP.md for convenience
- Added docs/operations/WARP.md to .gitignore
- But committed docs/operations/WARP.md BEFORE .gitignore took effect
...
# Unstage docs/operations/WARP.md
git reset HEAD docs/operations/WARP.md

# Commit only non-sensitive files
git commit -m "Fix import and add docs/operations/WARP.md to gitignore"
...
# Examples: docs/operations/WARP.md, .env.local, credentials.json
...
# docs/operations/WARP.md.example (safe to commit):
## GitHub Access Token
...
- ✅ docs/operations/WARP.md properly ignored with token safe locally
...
- `.gitignore` - Added docs/operations/WARP.md to prevent committing tokens (line 89)

**Documentation**:
- `docs/operations/WARP.md` - Added GitHub token section (local only, not committed)
...
- **Impact**:
  ...
  - Documentation burden (docs/operations/WARP.md, ARCHITECTURE.md, DESIGN_VALUE_CHART_ARCHITECTURE.md all referenced VALUE)
...
**Phase 2: AI Development Guidelines (docs/operations/WARP.md)**
...
2. **docs/operations/WARP.md** (added 150 lines) - AI development guidelines
...
- Added `seed:variables` (script removed in v12.3.30, messmass#352) command
...
```bash
`seed:variables` (script removed in v12.3.30, messmass#352)
```
...
**2. Updated Security Documentation** ([docs/security/security-enhancements.md](docs/security/security-enhancements.md)):
...
4. **Documentation Updates**
   - Updated `docs/operations/WARP.md` with file naming conventions and duplicate prevention guidelines
